...
DB_GRAPH_PATH  = "/home/zzp/ds_loo/hotel/hoteldata/turn_hotel_partial.jsonl"
QUERY_A_PATH   = "/home/zzp/ds_loo/hotel/hoteldata/dialogs_hotel_nondb.json"  # can be JSONL of turns; see reader below
LOCAL_MODEL_PATH = "/home/zzp/senbert"

# Output file (A')
OUTPUT_A_PATH = "/home/zzp/ds_loo/hotel/hoteldata/finetune_hotel_with_retrieval.json"

EXCEPT_DOMAIN = "hotel"      # <-- change to your target domain
MODE = "finetune"                 # "finetune" or "infer"

# Query events field switch (NO fallback/compat is performed):
# - "auto": finetune -> taklabels ; infer -> pred_taklabels
# - "taklabels" / "pred_taklabels": explicit override
QUERY_EVENT_FIELD = "auto"
...
def resolve_query_event_key(mode: str, query_event_field: str) -> str:
    qef = (query_event_field or "").strip()
    if qef and qef != "auto":
        if qef not in ("taklabels", "pred_taklabels"):
            raise ValueError(
                f"QUERY_EVENT_FIELD must be 'auto'/'taklabels'/'pred_taklabels', got: {query_event_field}"
            )
        return qef
    return "taklabels" if mode == "finetune" else "pred_taklabels"
...
QUERY_EVENT_KEY = resolve_query_event_key(MODE, QUERY_EVENT_FIELD)
print(f"[QUERY] MODE={MODE}, QUERY_EVENT_FIELD={QUERY_EVENT_FIELD} => using events from: {QUERY_EVENT_KEY}")
...
# 槽位所属域（比如 "hotel-area" -> "hotel"）
# 用 numpy 数组是为了后面方便做广播比较
q_slot_domains = np.array([get_domain(s) for s in q_slots], dtype=object) if q_slots else np.array([], dtype=object)
db_slot_domains = np.array([get_domain(s) for s in db_slots], dtype=object) if db_slots else np.array([], dtype=object)
...
# IMPORTANT: slot similarity uses ONLY slot NAME (not slot descriptions)
slot_texts = [s for s in needed_slots]

print(f"[EMB] encoding slot NAMES for {len(needed_slots)} slots ...")
...
for lab, cand in tqdm(label_bucket.items(), desc="Precompute by label", ncols=100):
    ...
    for col in range(N):
        ...
        # 训练阶段：强制跨域检索
        # 当前 query 槽位如果属于域 A，只允许使用“非 A 域”的候选槽位
        if MODE == "finetune" and Q > 0:
            # 候选槽位对应的域（长度 M）
            cand_db_domains = db_slot_domains[sidxs_arr]  # [M]
            # same_dom[q, m] = True 表示 q_slots[q] 与候选槽位域相同
            same_dom = (q_slot_domains[:, None] == cand_db_domains[None, :])
            allowed = ~same_dom
        else:
            # 推理阶段或者无约束时，不做域过滤
            allowed = np.ones_like(sims, dtype=bool)

        # 对于每个 query 槽位，是否在这个 turn 里有至少一个“跨域”候选槽位
        row_has_any = allowed.any(axis=1)  # [Q]

        if not np.any(row_has_any):
            # 这个 turn 对所有 query 槽位都没有合法候选，保持默认 -1
            continue

        # 只在 allowed=True 的位置上做 argmax，其余位置视为 -1e9
        masked_sims = np.where(allowed, sims, -1e9)
        arg = np.argmax(masked_sims, axis=1)   # [Q]
        maxv = masked_sims[np.arange(Q), arg]  # [Q]

        # 仅更新那些至少有一个合法候选槽位的行
        rows = np.where(row_has_any)[0]
        best_sim[rows, col] = maxv[rows]
        best_slot[rows, col] = sidxs_arr[arg[rows]]
...
# =========================
# Query file processing (SWITCHABLE events field)
# =========================
...
def flush_batch_turn_jsonl(out_fp, buffer_objs, buffer_ctx_texts):
    ...
    for obj, q_emb in zip(buffer_objs, q_ctx_embs):
        ...
        # Query events field is controlled by QUERY_EVENT_KEY (no fallback)
        events = obj.get(QUERY_EVENT_KEY)
        ...
        for ev in events:
            ...
            ev["retrieved_topk_ids"] = retrieve_topk_ids(
                q_dial_id=q_dial_id, q_label=q_label, q_slot=q_slot, q_ctx_emb=q_emb
            )
...
def annotate_jsonl_turn_file():
    ...
    with open(QUERY_A_PATH, "r", encoding="utf-8") as fin, open(OUTPUT_A_PATH, "w", encoding="utf-8") as fout:
        ...
        for line in fin:
            ...
            # Only turns WITH non-empty events under QUERY_EVENT_KEY will trigger retrieval.
            evs = obj.get(QUERY_EVENT_KEY)
            need_retrieval = isinstance(evs, list) and (len(evs) > 0)

            if not need_retrieval:
                # Keep output order: flush buffered (earlier) retrieval turns before writing this line.
                if buffer_objs:
...
def annotate_dialog_json_file():
    # JSON list of dialogs: keep original structure; add retrieved_topk_ids into each turn's events under QUERY_EVENT_KEY
    ...
    with open(OUTPUT_A_PATH, "w", encoding="utf-8") as fout:
        ...
        for d in tqdm(dialogs, desc="Annotating dialog JSON with retrieval IDs", ncols=110):
            ...
            # build ctx for turns that have non-empty events under QUERY_EVENT_KEY
            ctx_texts: List[str] = []
            turn_indices: List[int] = []

            for ti, t in enumerate(turns):
                evs = t.get(QUERY_EVENT_KEY)
            ...
            if ctx_texts:
                ...
                for (ti, emb) in zip(turn_indices, ctx_embs):
                    evs = turns[ti].get(QUERY_EVENT_KEY)
...
print(
    f"MODE={MODE}, EXCEPT_DOMAIN={EXCEPT_DOMAIN}, QUERY_EVENT_FIELD={QUERY_EVENT_FIELD}, QUERY_EVENT_KEY={QUERY_EVENT_KEY}, "
    f"TOPK={TOPK}, FORBID_SAME_DIAL={FORBID_SAME_DIAL}, USE_LABEL2_REFER_IMPLICIT={USE_LABEL2_REFER_IMPLICIT}"
)
